...
Raw 的全部模式按 `page=1, 2, 3…` 取原文，不会逐层请求 Raw；同时会读取楼层作者索引，以恢复分页块中的可点击作者链接。仅一楼和精确楼层范围只请求所选楼层；仅贴主模式需要先读楼层索引，才能筛出贴主回复。相对图片和链接会自动补上站点域名；选择 Base64 或 WebDAV 时会改写 Markdown 中的图片链接。
